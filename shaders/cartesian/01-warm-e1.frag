...
vec3 one = vec3(0.4, 0.0, 0.3);
vec3 two = vec3(0.8, 0.0, 0.1);
...
void main(){
  ...
  float f3 = 0.5 + sin(st.x * 4.0 - u_time) * 0.25;
  float f4 = 0.5 + sin(st.x * 32.0 * u_energy + u_time) * 0.5;
  pct.r = f3 * f4;
  ...
  // color = mix(color, vec3(0.8, 0.8, 0.0), ploty(st, pct.g));
